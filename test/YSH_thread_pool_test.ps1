#!/usr/bin/env pwsh
$Host.UI.RawUI.WindowTitle = 'YSH::thread_pool Test Script'
...
Write-Title '=== YSH::thread_pool test script ==='

$SourceFile = 'YSH_thread_pool_test.cpp'
...
$ClangExe = 'YSH_thread_pool_test_clang' + $Extension
$GCCExe = 'YSH_thread_pool_test_gcc' + $Extension

$MSVCName = 'YSH_thread_pool_test_msvc'
...
If ($IsWindows)
{
    ...
    If (Test-Path $MSVCEnv)
    {
        ...
        $Env:YSH_THREAD_POOL_MSVC_COMMAND = "cl $SourceFile /std:c++17 /permissive- /O2 /W4 /EHsc /Fe:$FullMSVCExe /Fo:$FullMSVCObj"
        Write-Command $Env:YSH_THREAD_POOL_MSVC_COMMAND
        pwsh -Command {
            $CurrentDir = Get-Location
            & 'C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\Tools\Launch-VsDevShell.ps1'
            Set-Location $CurrentDir
            Invoke-Expression $Env:YSH_THREAD_POOL_MSVC_COMMAND
            Exit $LASTEXITCODE
        }
        $Env:YSH_THREAD_POOL_MSVC_COMMAND = $Null
        If ($LASTEXITCODE)
        {
            Write-Error "Failed to compile, aborting script! (Exit code: $LASTEXITCODE)"
            Exit-Script $LASTEXITCODE
