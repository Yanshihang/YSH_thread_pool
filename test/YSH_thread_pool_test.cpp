 ...
 *
 * @brief YSH::thread_pool: a fast, lightweight, and easy-to-use C++17 thread pool library. This program tests all aspects of the library, but is not needed in order to use the library.
 */
...
//#include "../include/YSH_thread_pool_light.hpp"
...
// A global synced_out_stream object which prints to std::cout.
YSH::synced_out_stream sync_cout(std::cout);

// A global stream object used to access the log file.
std::ofstream log_file;

// A global synced_out_stream object which prints to the log file.
YSH::synced_out_stream sync_file(log_file);

// Global thread pool objects to be used throughout the test.
YSH::thread_pool pool_full;
//YSH::thread_pool_light pool_light;
...
{
    dual_print(std::forward<T>(items)..., YSH::synced_out_stream::endl);
}
...
YSH::concurrency_t count_unique_threads(P& pool)
{
    ...
    std::mutex ID_mutex, total_mutex;
    {
        ...
        total_cv.wait(total_lock, [&total_count, &pool] { return total_count == pool.get_threads_num(); });
        {
        ...
        return static_cast<YSH::concurrency_t>(std::unique(thread_IDs.begin(), thread_IDs.end()) - thread_IDs.begin());
    }
}
 ...
 * @param pool The thread pool to check.
 */
void check_reset(YSH::thread_pool& pool)
{
    ...
    check(std::thread::hardware_concurrency() / 2, pool.get_threads_num());
...
YSH::thread_pool check_deadlock_pool;
...
{
    const YSH::concurrency_t n = pool.get_threads_num() * 10;
    std::unique_ptr<std::atomic<bool>[]> flags = std::make_unique<std::atomic<bool>[]>(n);
    for (YSH::concurrency_t i = 0; i < n; ++i)
        pool.push_task(
            [&flags, i]
            {
...
void check_wait_for_tasks_blocks(P& pool)
{
    ...
    for (YSH::concurrency_t i = 0; i < num_waiting_tasks; ++i)
 ...
 * @param pool The thread pool to check.
 */
void check_wait_for_tasks_duration(YSH::thread_pool& pool)
{
 ...
 * @param pool The thread pool to check.
 */
void check_wait_for_tasks_until(YSH::thread_pool& pool)
{
...
void check_parallelize_loop_no_return(P& pool, const int64_t random_start,T random_end, const YSH::concurrency_t num_tasks, const bool use_push = false)
{
    ...
    else if constexpr (std::is_same_v<P, YSH::thread_pool>)
    {
        if (random_start == 0)
 ...
 * @param num_tasks The number of tasks.
 */
void check_parallelize_loop_return(YSH::thread_pool& pool, const int64_t random_start, int64_t random_end, const YSH::concurrency_t num_tasks)
{
    if (random_start == random_end)
        ++random_end;
...
void check_loop(P& pool)
{
    ...
    std::uniform_int_distribution<YSH::concurrency_t> task_dist(1, pool.get_threads_num());
    constexpr uint64_t n = 10;
    if constexpr (std::is_same_v<P, YSH::thread_pool>)
    {
 ...
 * @param pool The thread pool to check.
 */
void check_task_monitoring(YSH::thread_pool& pool)
{
    YSH::concurrency_t n = std::min<YSH::concurrency_t>(std::thread::hardware_concurrency(), 4);
    ...
    for (YSH::concurrency_t i = 0; i < n * 3; ++i)
        pool.push_task(
            [&release, i]
            {
                while (!release[i])
    ...
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == n * 3 && pool.get_tasks_running_num() == n && pool.get_tasks_queued() == n * 2);
    for (YSH::concurrency_t i = 0; i < n; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
    ...
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == n * 2 && pool.get_tasks_running_num() == n && pool.get_tasks_queued() == n);
    for (YSH::concurrency_t i = n; i < n * 2; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);

    dual_println("After releasing ", n, " more tasks, should have: ", n, " tasks total, ", n, " tasks running, ", 0, " tasks queued...");
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == n && pool.get_tasks_running_num() == n && pool.get_tasks_queued() == 0);
    for (YSH::concurrency_t i = n * 2; i < n * 3; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);

    dual_println("After releasing the final ", n, " tasks, should have: ", 0, " tasks total, ", 0, " tasks running, ", 0, " tasks queued...");
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == 0 && pool.get_tasks_running_num() == 0 && pool.get_tasks_queued() == 0);
...
void check_pausing(YSH::thread_pool& pool)
{
    ...
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == n * 3 && pool.get_tasks_running_num() == 0 && pool.get_tasks_queued() == n * 3);
    ...
    dual_print("Result: ", pool.get_total_tasks_num(), " tasks total, ", pool.get_tasks_running_num(), " tasks running, ", pool.get_tasks_queued(), " tasks queued ");
    check(pool.get_total_tasks_num() == n && pool.get_tasks_running_num() == 0 && pool.get_tasks_queued() == n);
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
 ...
 * @param pool The thread pool to check.
 */
void check_purge(YSH::thread_pool& pool)
{
    ...
    check(done[0]);
    bool any_flag_set = false;
    for (YSH::concurrency_t i = 1; i < num_tasks; ++i)
...
/**
 * @brief Check that exceptions are forwarded correctly by YSH::multi_future.
 *
 * @param pool The thread pool to check.
 */
void check_exceptions_multi_future(YSH::thread_pool& pool)
{
    dual_println("Checking that exceptions are forwarded correctly by YSH::multi_future...");
    bool caught = false;
    auto throws = []
    {
        dual_println("Throwing exception...");
    ...
    YSH::multi_future<void> my_future;
 ...
 */
template <typename P>
void check_vector_of_size(P& pool, const size_t vector_size, const YSH::concurrency_t num_tasks)
{
...
//    check_member_function_within_object(pool_light);
    ...
    check_wait_for_tasks_deadlock<YSH::thread_pool>();
...
//    check_wait_for_tasks(pool_light);
    check_wait_for_tasks_blocks(pool_full);
//    check_wait_for_tasks_deadlock<YSH::thread_pool_light>();
 ...
 */
void print_timing(const YSH::concurrency_t num_tasks, const std::pair<double, double>& mean_sd)
{
    if (num_tasks == 0)
 ...
 */
void print_speedup(const std::vector<double>& timings, const YSH::concurrency_t try_tasks[])
{
    ...
    const YSH::concurrency_t num_tasks = try_tasks[min_el - std::begin(timings)];
...
void check_performance()
{
    ...
    const YSH::concurrency_t thread_count = pool.get_threads_num();
    dual_println("Using ", thread_count, " threads.");

    // Define the number of tasks to try in each run of the test (0 = single-threaded).
    const YSH::concurrency_t try_tasks[] = {0, thread_count / 4, thread_count / 2, thread_count, thread_count * 2, thread_count * 4};
    ...
    do
    {
        ...
        pool.wait_for_tasks();
        tmr.end();
    } while (tmr.get_ms() < target_ms);
    num_vectors = thread_count * static_cast<size_t>(std::llround(static_cast<double>(num_vectors) * static_cast<double>(target_ms) / static_cast<double>(tmr.get_ms()) / thread_count));
    ...
    for (YSH::concurrency_t n : try_tasks)
    {
        for (size_t r = 0; r < repeat; ++r)
        {
            ...
            tmr.end();
            same_n_timings.push_back(tmr.get_ms());
        }
...
int main()
{
    const std::string log_filename = "YSH_thread_pool_test-" + get_time() + ".log";
    if (output_log)
        log_file.open(log_filename);

    dual_println("YSH::thread_pool: a fast, lightweight, and easy-to-use C++17 thread pool library");
    ...
    dual_println("Thread pool library version is ", YSH_THREAD_POOL_VERSION, ".");
//    dual_println("Light thread pool library version is ", YSH_THREAD_POOL_LIGHT_VERSION, ".");
